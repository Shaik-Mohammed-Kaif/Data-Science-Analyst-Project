...
#
# NOTE: This version trains models in-session (st.session_state)
# and does NOT use joblib / disk-based model persistence.
# ================================================================
...
# ================================================================
# SESSION STATE INITIALIZATION (models live only in session memory)
# ================================================================
...
if "cache_version" not in st.session_state:
    # Bumping this forces train_models_cached() to treat the next
    # call as a brand-new cache key, i.e. a real retrain, even if the
    # data/features/test_size are otherwise identical.
    st.session_state.cache_version = 0

if "ml_auto_trained_once" not in st.session_state:
    st.session_state.ml_auto_trained_once = False
...
def get_active_model_bundle():
    """
    Get the trained model from the current Streamlit session.

    No model file is loaded from disk.
    No .joblib file is created.
    If the session restarts, the model must be retrained from
    ML Model Lab. This is intentional.
    """
...
def _make_data_signature(df, feature_cols, target_col):
    """
    Build a lightweight, hashable fingerprint of the exact data used
    for training (selected features + target only). This is what
    st.cache_resource uses as the cache key — if the fingerprint is
    identical, the cached model is reused instantly with NO retraining.
    If the underlying data or the selected columns change, the
    fingerprint changes automatically and a fresh model is trained.
    """
    ...
    subset = df[cols]
    try:
        return int(pd.util.hash_pandas_object(subset, index=True).sum())
    except Exception:
        # Fallback fingerprint if hashing fails for any reason
        return hash((subset.shape, tuple(subset.columns), str(subset.values.tobytes()[:2048])))


@st.cache_resource(show_spinner="Training classification models (first time only — cached after this)...")
def train_models_cached(data_signature, feature_cols, test_size, target_col, cache_version, _df):
    """
    Cached training wrapper.

    NOTE ON ARGUMENTS:
    - `_df` starts with an underscore, so Streamlit does NOT hash it and
      does NOT use it to decide cache hits/misses (it's just the data
      the function needs to actually run).
    - `data_signature`, `feature_cols`, `test_size`, `target_col` and
      `cache_version` ARE hashed and together form the cache key.

    Effect:
    - Same data + same features + same test size + same cache_version
      -> returns the ALREADY TRAINED model instantly, no retraining.
    - Anything in the key changes (new/edited data, different feature
      selection, different test size, or cache_version bumped via the
      "Force Retrain" button) -> a fresh model is trained automatically.
    - The trained model lives in the Streamlit server process's memory
      for as long as the app process stays alive — no .joblib file is
      ever written to or read from disk.
    """
    return train_models(_df, target_col, list(feature_cols), test_size)
...
if page == "Home":
    ...
    with st.container(border=True):
        ...
        if st.session_state.get("model_results") is None:
            st.warning(
                "⚠️ No model is trained yet in this session. "
                "Go to **ML Model Lab** and click **Train / Retrain "
                "Classification Models** to begin."
            )
        else:
            st.success(
                f"✅ A model is trained for this session: "
                f"**{st.session_state.model_results['best_name']}**"
            )
...
# ================================================================
    ...
    # ============================================================
    # CURRENT MODEL STATUS (SESSION ONLY — NO DISK PERSISTENCE)
    # ============================================================
    ...
    if existing_bundle:
        ...
        st.success(
            f"✅ Trained model available for this session: **{best_name}**"
        )
        ...
        st.info(
            "ℹ️ No model has been trained yet in this session. "
            "Train the models once below — the model stays available "
    ...
    st.caption(
        "🧠 **One-time training:** once a model has been trained for a "
        "given data + feature + test-size combination, it is cached in "
        "the app's memory (`st.cache_resource`). Reopening the app, "
        "reloading the page, or other users visiting the app will reuse "
        "the SAME trained model instantly — no retraining, no `.joblib` "
        "file. Training only happens again if you change the data, the "
        "selected features, the test size, or press **Force Retrain**."
    )

    # ============================================================
    # AUTO-TRAIN ON FIRST VISIT TO THIS PAGE (no button click needed)
    # ============================================================
    # Runs once per browser session using the current default
    # feature set + test size. If a cached model for this exact
    # combination already exists anywhere in the running app
    # process, it is fetched instantly with no retraining.
    # ============================================================

    if (
        not st.session_state.ml_auto_trained_once
        and st.session_state.model_results is None
        and candidate_features
    ):

        try:

            auto_signature = _make_data_signature(
                filtered,
                candidate_features,
                TARGET_COLUMN
            )

            auto_bundle = train_models_cached(
                auto_signature,
            ...
            st.session_state.model_results = auto_bundle
            st.session_state.trained_bundle = auto_bundle

        except Exception:
            # If anything goes wrong here, fall through silently —
            # the user can still train manually with the button below.
            pass

        st.session_state.ml_auto_trained_once = True

    # ============================================================
    # TRAIN / RETRAIN (cached — instant if already computed)
    # ============================================================

    train_col, retrain_col = st.columns([3, 1])
    ...
    with retrain_col:
        ...
        force_retrain_button = st.button(
            "♻️ Force Retrain",
            use_container_width=True,
            help="Use this after you update the CSV or change the "
                 "training logic, to bypass the cache and train fresh."
        )

    if force_retrain_button:
        st.session_state.cache_version += 1
        st.info(
            f"🔄 Cache version bumped to "
            f"{st.session_state.cache_version}. Click "
            f"**Train Classification Models** to train a fresh model."
        )
    ...
    if train_button:
        ...
        if not selected_features:
            ...
            try:
                ...
                signature = _make_data_signature(
                    filtered,
                    selected_features,
                    TARGET_COLUMN
                )
                ...
                # ------------------------------------------------
                # STORE IN SESSION FOR THIS USER'S CURRENT VIEW
                # (the underlying trained model itself lives in
                # st.cache_resource, shared across the whole app)
                # ------------------------------------------------
                ...
                st.success(
                    f"✅ Model ready. Best model by F1: "
                    f"**{bundle['best_name']}**"
                )

                st.info(
                    "🧠 This model is cached for the current data/feature/"
                    "test-size combination. It will be reused instantly "
                    "next time — no retraining needed — until the data "
                    "changes or you press Force Retrain."
                )
    ...
    if bundle is None:
        ...
        st.caption(
            ...
            "once to train a model for this session."
        )
        ...
        # ============================================================
        # MODEL SESSION STATUS (replaces disk-file status card)
        # ============================================================

        st.markdown(
            '<div class="section-label">MODEL SESSION STATUS</div>',
            unsafe_allow_html=True
        )
        ...
        with storage_cols[0]:
            ...
            card_kpi(
                "MODEL LOCATION",
                "CACHED",
                "st.cache_resource — no .joblib file",
                "🧠"
            )
        ...
        with storage_cols[2]:
            ...
            card_kpi(
                "CACHE VERSION",
                f"{st.session_state.cache_version}",
                "Bumped only by Force Retrain",
                "🔢"
            )
...
# ================================================================
    ...
    # ============================================================
    # GET MODEL FROM SESSION ONLY (no disk fallback)
    # ============================================================
    ...
    if bundle is None:
        ...
        st.warning(
            "⚠️ No trained model is currently available in this session."
        )

        st.info(
            "Go to **ML Model Lab → Train / Retrain Classification Models** "
            "once. The trained model will stay available in this session "
            "for Prediction Studio, Model Evaluation and Feature Intelligence."
        )
...
# ================================================================
    ...
    if bundle is None:
        ...
        st.info(
            "Train a model once from **ML Model Lab** to unlock "
            "Feature Intelligence for this session."
        )
    ...
    # FEATURE IMPORTANCE
    # ============================================================
    # Reuse cached importance inside the current session bundle
    # whenever available, so it isn't recomputed on every rerun.
    # ============================================================
    ...
    if (
        ...
        # --------------------------------------------------------
        # Cache calculated importance inside the session bundle.
        # No file is written to disk — this only updates memory.
        # --------------------------------------------------------
    ...
    st.info(
        f"The current session's trained model is **{best_name}**. "
...
# ================================================================
    ...
    with st.container(border=True):
        ...
        st.write(
            """
            **Model persistence note:** this application trains
            models live inside your Streamlit session
            (`st.session_state`). No `.joblib` file is written to
            or read from disk, so the app stays lightweight to
            deploy. If the app restarts, simply retrain from
            **ML Model Lab**.
            """
        )
...
if page != "About":
    ...
    st.info(
        ...
        "evaluation context.\n\n"
        "Models are trained live in-session and are not persisted "
        "to disk as `.joblib` files."
    )
